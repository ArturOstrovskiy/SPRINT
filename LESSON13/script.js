const list = document.querySelector('.list') //присваивает переменной list html элемент с классом list, а именно <ol class="list">…</ol>

list.addEventListener('click', function (event) { //по клику передается параметр event в функцию
	const targetElement = event.target //присваиваем targetElement значение event, а .target указывает на конкретный элемент
...
form.addEventListener('submit', function (event) { //по клику на элемент submit передается параметр event в функцию
	event.preventDefault() //.preventDefault() это метод который прерывает стандартное поведение браузера связанное с этим событием
